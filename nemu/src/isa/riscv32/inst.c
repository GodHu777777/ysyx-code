...
#define immB() do { *imm = ( (SEXT(BITS(i, 31, 31), 1) << 12) | (BITS(i, 30, 25) << 5) | (BITS(i, 11, 8) << 1) | BITS(i, 7, 7) << 11); Log("imm RAW: %x", *imm);} while(0)
              // ( (1 << 13) - 1) & 
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 000 ???? ? 1100011", beq    , B, s->dnpc += (src1 == src2) ? (imm - 4) : 0;Log("[BEQ DEBUG]: isEqual: %d, imm: %d i:", src1 == src2, imm));
  ...
  INSTPAT("0000001 ????? ????? 100 ????? 011 0011", div    , R, R(rd) = src1 / src2);
  ...
  INSTPAT("???? ???? ???? ???? ???? ????? 1101111", jal    , J, R(rd) = s->pc + 4;s->dnpc += imm - 4; Log("imm: %x",imm)); 
